...
//
// PS: 为啥正着 wiggle 不行呢?
// 根据题目保证可以得到以下结论: 排序后对任意的 `i`, `a[mid+i] != a[i]`, `mid = (n + 1) // 2` (反证, 如果 `a[mid+i] == a[i]`, `a[i]` 这个值就超一半了. 超一半的话就会出现至少两个 `a[i]` 挨着了, 不符合要求)
// 那么, 需要让 wiggle 后相邻两项 `a[i], a[i+1]` 在排序前 `b[i], b[j]` 满足 `|j-i| >= mid`. 这时你发现从小到大 wiggle 的时候可能会不满足条件, 反例: `[4,5,5,6]`.
/*
