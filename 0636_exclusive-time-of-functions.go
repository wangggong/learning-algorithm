/*
 * @lc app=leetcode.cn id=exclusive-time-of-functions lang=golang
 *
...
func exclusiveTime(n int, logs []string) []int {
	...
	for _, l := range logs {
		...
		case "start":
			if len(S) > 0 {
				timer[S[len(S)-1][0]] += t - S[len(S)-1][1]
			}
			S = append(S, [2]int{i, t})
		case "end":
			if len(S) > 0 {
				timer[S[len(S)-1][0]] += t - S[len(S)-1][1] + 1
			}
			S = S[:len(S)-1]
			if len(S) > 0 {
				S[len(S)-1][1] = t + 1
