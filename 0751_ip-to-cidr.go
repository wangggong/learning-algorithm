...
func ipToCIDR(ip string, n int) []string {
	...
	for n > 0 {
		mask := 33 - min(
			bitLength(start&-start), // 最后一个 `1` 的位数, 此时凑不够 `n` 个地址, 因为前一个地址不在里面
			bitLength(n),            // `n` 个地址拉满对应的位数, 全来吧
		)
		ans = append(ans, int2ip(start)+"/"+strconv.Itoa(mask))
		diff := 1 << (32 - mask)
		start += diff
		n -= diff
	}
...
func int2ip(x int) string {
	mask := 1<<8 - 1
	parts := []string{
		strconv.Itoa(x >> 24),
		strconv.Itoa((x >> 16) & mask),
		strconv.Itoa((x >> 8) & mask),
		strconv.Itoa(x & mask),
	}
...
func bitLength(x int) int {
	// NOTE: 一个奇怪的补丁: 对应 case `"0.0.0.0"\n1\n`.
	// 其实就是只要起点是从 0 开始的, mask 仅受到 `n` 的位数约束.
	if x == 0&-0 {
		return 32
	}
	k := 0
