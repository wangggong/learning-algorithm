/*
 * @lc app=leetcode.cn id=champagne-tower lang=csharp
 *
...
public class Solution
{
    ...
    {
        double[,] glasses = new double[N + 1, N + 1];
        // 装作把所有香槟都倒在最上面的杯子中, 往下逐层分配.
        glasses[0, 0] = poured;
        for (int i = 1; i <= row + 1; i++)
        {
            for (int j = 0; j <= i; j++) { glasses[i, j] = (j > 0 ? Math.Max(glasses[i - 1, j - 1] - 1, 0) / 2 : 0) + (j < i ? Math.Max(glasses[i - 1, j] - 1, 0) / 2 : 0); }
        }
        return Math.Min(glasses[row, col], 1);
    }
}
