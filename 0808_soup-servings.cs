/*
 * @lc app=leetcode.cn id=soup-servings lang=csharp
 *
...
public class Solution
{
    ...
    public double SoupServings(int n)
    {
        ...
        double dfs(int a, int b)
        {
            if (a == 0 || b == 0)
            {
                return a == 0 && b == 0 ? .5 : (a == 0 ? 1 : 0);
            }
            var key = (a, b);
            if (!memo.ContainsKey(key))
            {
                memo[key] = 0;
                foreach (var srv in Servings)
                {
                    memo[key] += dfs(Math.Max(a - srv[0], 0), Math.Max(b - srv[1], 0)) * .25;
                }
            }
            return memo[key];
        }
