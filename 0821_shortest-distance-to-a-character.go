/*
 * @lc app=leetcode.cn id=shortest-distance-to-a-character lang=golang
 *
...
func shortestToChar(s string, c byte) []int {
	...
	for p < n {
		...
		for q = p + 1; q < n && arr[q] != c; q++ {
		}
		if q == n {
			break
		}
		cnt := 0
		for i, j := p+1, q-1; i <= j; i, j = i+1, j-1 {
			cnt++
			ans[i] = cnt
			ans[j] = cnt
		}
 ...
 * 		q := Q[0]
 * 		Q = Q[1:]
 * 		if q+1 < n && ans[q+1] == math.MaxInt32 {
 * 			ans[q+1] = ans[q] + 1
 * 			Q = append(Q, q+1)
 * 		}
 * 		if q-1 >= 0 && ans[q-1] == math.MaxInt32 {
 * 			ans[q-1] = ans[q] + 1
 * 			Q = append(Q, q-1)
 * 		}
 * 	}
