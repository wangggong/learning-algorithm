/*
 * @lc app=leetcode.cn id=count-unique-characters-of-all-substrings-of-a-given-string lang=golang
 *
...
func uniqueLetterString(s string) (ans int) {
	for a := byte('A'); a <= 'Z'; a++ {
		...
		ind = append(ind, len(s))
		for i, m, cur := 0, len(ind), -1; i+1 < m; i++ {
			ans += (ind[i] - cur) * (ind[i+1] - ind[i])
			cur = ind[i]
		}
	}
	return
