/*
 * @lc app=leetcode.cn id=reorder-data-in-log-files lang=golang
 *
...
func reorderLogFiles(logs []string) []string {
	var alphabets, numbers []string
	for _, l := range logs {
		if isAlphabet(l) {
			alphabets = append(alphabets, l)
		} else {
			numbers = append(numbers, l)
		}
	}
	sort.Slice(alphabets, func(p, q int) bool {
		return cmpAlphabet(alphabets[p], alphabets[q])
	})
	var ans []string
	for _, l := range alphabets {
		ans = append(ans, l)
	}
...
func isAlphabet(s string) bool {
	i := strings.Index(s, " ")
	return 'a' <= s[i+1] && s[i+1] <= 'z'
}

func cmpAlphabet(s, t string) bool {
	i, j := strings.Index(s, " "), strings.Index(t, " ")
	tags, contents := s[:i], s[i+1:]
	tagt, contentt := t[:j], t[j+1:]
	if contents != contentt {
		return contents < contentt
	}
	return tags < tagt
}
