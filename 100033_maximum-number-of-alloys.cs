/*
 * @lc app=leetcode.cn id=maximum-number-of-alloys lang=csharp
 *
...
{
    private const int N = (int)4e8;
    
    private bool Check(int k, int budget, IList<int> composition, IList<int> stock, IList<int> cost)
        => composition
            .Zip(stock, (c, s) => Math.Max((long)c * (long)k - (long)s, 0))
            .Zip(cost, (n, c) => (long)n * (long)c)
            .Sum() <= (long)budget;


    public int MaxNumberOfAlloys(int n, int k, int budget, IList<IList<int>> compositions, IList<int> stock, IList<int> cost) => compositions
        .Select(composition =>
        {
            var (p, q) = (0, N);
            while (p < q)
            {
                var mid = (p + q) >> 1;
                if (!Check(mid, budget, composition, stock, cost)) { q = mid; }
                else { p = mid + 1; }
            }
            return p - 1;
        })
