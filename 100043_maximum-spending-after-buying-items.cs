/*
 * @lc app=leetcode.cn id=maximum-spending-after-buying-items lang=csharp
 *
...
public class Solution
{
    public long MaxSpending(int[][] values)
    {
        ...
        for (var k = 0; k < m; k++) { Q.Enqueue((k, n - 1), values[k][n - 1]); }
        var ans = 0l;
        for (var t = 1l; Q.Count > 0; t++)
        {
            var (i, j) = Q.Dequeue();
            ans += t * (long)values[i][j];
            if (j > 0) { Q.Enqueue((i, j - 1), values[i][j - 1]); }
        }
        return ans;
    }
}
