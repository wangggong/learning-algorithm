/*
 * @lc app=leetcode.cn id=minimum-cost-to-convert-string-i lang=csharp
 *
...
public class Solution
{
    public long MinimumCost(string source, string target, char[] original, char[] changed, int[] cost)
    {
        ...
        int index(char c) => (int)(c - 'a');
        for (var (i, m) = (0, original.Length); i < m; i++)
        {
            var (u, v, w) = (index(original[i]), index(changed[i]), cost[i]);
        ...
        floyd();
        var ans = source
            .Zip(target, (s, t) => G[index(s)][index(t)])
            .Sum();
        return ans >= Maxn
            ? -1
            : ans;
