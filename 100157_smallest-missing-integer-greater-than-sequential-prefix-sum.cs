/*
 * @lc app=leetcode.cn id=smallest-missing-integer-greater-than-sequential-prefix-sum lang=csharp
 *
...
public class Solution
{
    ...
    {
        var ans = nums[0];
        for (var (i, n) = (1, nums.Length); i < n && nums[i] - 1 == nums[i - 1]; i++)
        { ans += nums[i]; }
        for (var S = nums.ToHashSet(); S.Contains(ans); ans++) { }
        return ans;
    }
}
