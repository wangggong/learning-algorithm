#
# @lc app=leetcode.cn id=minimum-cost-to-convert-string-ii lang=python3
#
...
# 参考: https://leetcode.cn/problems/minimum-cost-to-convert-string-ii/solutions/2577896/xiao-yang-xiao-en-li-kou-bu-deng-shi-gun-703w/
#
# 曾经我相信科学, 但特么这玩意真卡常啊!
class Solution:
    def minimumCost(self, source: str, target: str, original: List[str], changed: List[str], cost: List[int]) -> int:
        ...
        for x, y, z in zip(original, changed, cost):
            G[x][y] = z if y not in G[x] else min(G[x][y], z)
        dist = {}
        for x in G:
            dist[x] = {}
            dist[x][x] = 0
            ...
            while Q:
                d, u = heappop(Q)
                if dist[x][u] == d and u in G:
                    for v in G[u]:
                        if dist[x].get(v, inf) > d + G[u][v]:
                            dist[x][v] = d + G[u][v]
                            heappush(Q, (dist[x][v], v))
        n = len(source)
        dp = [inf] * (n + 1)
        dp[0] = 0
        for i in range(n):
            ...
            for v in dist:
                l = len(v)
                if i + 1 >= l and l not in vis:
                    s, t = source[i - l + 1 : i + 1], target[i - l + 1 : i + 1]
                    if s in dist and t in dist[s]:
                        dp[i + 1] = min(dp[i + 1], dp[i - l + 1] + dist[s][t])
                        vis.add(l)
