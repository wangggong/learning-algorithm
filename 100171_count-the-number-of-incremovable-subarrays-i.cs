/*
 * @lc app=leetcode.cn id=count-the-number-of-incremovable-subarrays-i lang=csharp
 *
...
{
    private bool Check(int[] arr, int p, int q)
    {
        var lists = new List<int>();
        lists.AddRange(arr[..p]);
        lists.AddRange(arr[(q + 1)..]);
        return !lists.Any()
            ? true
            : Enumerable
                .Range(1, lists.Count() - 1)
                .All(i => lists[i] > lists[i - 1]);
    }

    public int IncremovableSubarrayCount(int[] nums) => Enumerable
        .Range(0, nums.Length)
        .Select(i => Enumerable
            .Range(i, nums.Length - i)
            .Count(j => Check(nums, i, j)))
        .Sum();
}
