/*
 * @lc app=leetcode.cn id=find-beautiful-indices-in-the-given-array-ii lang=csharp
 *
...
public class Solution
{
    public IList<int> BeautifulIndices(string s, string a, string b, int k)
    {
        ...
        for (var i = 0; i < n; i++)
        {
            hashs[i + 1] = hashs[i] * P + (long)(s[i] - 'a');
            muls[i + 1] = muls[i] * P;
        }
        var (ha, hb) = (0l, 0l);
        for (var i = 0; i < na; i++) { ha = ha * P + (long)(a[i] - 'a'); }
        for (var i = 0; i < nb; i++) { hb = hb * P + (long)(b[i] - 'a'); }
        var indexes = Enumerable.Range(0, n - nb + 1)
            .Where(i => hashs[i + nb] - hashs[i] * muls[nb] == hb)
                // Don't check, will TLE
                // && s[i..(i + nb)] == b)
            .ToList();
        var m = indexes.Count();
        return Enumerable.Range(0, n - na + 1)
            .Where(i => hashs[i + na] - hashs[i] * muls[na] == ha)
                // && s[i..(i + na)] == a)
            .Where(i =>
            {
                var (p, q) = (0, m);
