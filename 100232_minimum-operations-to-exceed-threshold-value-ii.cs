/*
 * @lc app=leetcode.cn id=minimum-operations-to-exceed-threshold-value-ii lang=csharp
 *
...
public class Solution
{
    public int MinOperations(int[] nums, int k)
    {
        ...
        foreach (var n in nums) { Q.Enqueue((long)n, (long)n); }
        var ans = 0;
        for (; Q.Count > 1 && Q.Peek() < k; ans++)
        {
            var x = Q.Dequeue();
            var y = Q.Dequeue();
            if (x > y) { (x, y) = (y, x); }
