/*
 * @lc app=leetcode.cn id=most-frequent-ids lang=csharp
 *
 ...
 * 
 * 
 * 请你返回一个长度为 n 的数组 ans ，其中 ans[i] 表示第 i 步操作后出现频率最高的 ID 数目 ，如果在某次操作后集合为空，那么
 * ans[i] 为 0 。
 * 
 * 
 * 
...
public class Solution
{
    public long[] MostFrequentIDs(int[] nums, int[] freq)
    {
        ...
        var Q = new PriorityQueue<int, long>();
        var ans = new List<long>();
        foreach (var (n, f) in nums.Zip(freq))
        {
            d.TryGetValue(n, out var c);
            c += f;
            d[n] = c;
            Q.Enqueue(n, -c);
            for (; true; Q.Dequeue())
            {
                Q.TryPeek(out var v, out c);
                if (d[v] == -c)
                {
                    ans.Add(-c);
                    break;
