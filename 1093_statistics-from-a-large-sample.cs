/*
 * @lc app=leetcode.cn id=statistics-from-a-large-sample lang=csharp
 *
...
{
    public double[] SampleStats(int[] count) => new[]
    {
        (double)(count.Select((c, i) => (c, i)).Where(v => v.c != 0).First().i),
        (double)(count.Select((c, i) => (c, i)).Where(v => v.c != 0).Last().i),
        (double)(count.Select((c, i) => (long)c * (long)i).Sum())
            / (double)(count.Sum()),
        GetMedian(count.Select((c, i) => (c, i)).Where(v => v.c != 0).ToArray()),
        (double)(count.Select((c, i) => (c, i)).OrderBy(v => -v.c).First().i),
    };

    private double GetMedian((int, int)[] count)
    {
        var total = count.Select(v => v.Item1).Sum();
        for (var (i, cur) = (0, 0); true; i++)
        {
            cur += count[i].Item1;
            if (cur > total / 2)
            {
                return total % 2 == 1
                    ? (double)count[i].Item2
                    : ((double)count[i].Item2
                        + (double)(cur - count[i].Item1 < total / 2
                            ? count[i].Item2
                            : count[i - 1].Item2)) / 2;
            }
        }
