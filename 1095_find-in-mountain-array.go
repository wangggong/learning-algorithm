/*
 * @lc app=leetcode.cn id=find-in-mountain-array lang=golang
 *
...
func findInMountainArray(target int, ma *MountainArray) int {
	...
	// 获取到峰值及其对应 index, 如果 `peak == target`, 直接返回...
	peak := ma.get(l)
	peakInd := l
	if target == peak {
		return peakInd
	...
	// 在左边找一波...
	for l, r := 0, peakInd-1; l <= r; {
		mid := (l + r) >> 1
		if v := ma.get(mid); v > target {
			r = mid - 1
		} else if v < target {
			l = mid + 1
		} else {
