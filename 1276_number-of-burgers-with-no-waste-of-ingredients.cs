/*
 * @lc app=leetcode.cn id=number-of-burgers-with-no-waste-of-ingredients lang=csharp
 *
...
{
    public IList<int> NumOfBurgers(int x, int y) => x >= 2 * y
        && (x - 2 * y) % 2 == 0
        && 4 * y >= x
        && (4 * y - x) % 2 == 0
        ? new List<int> { (x - 2 * y) / 2, (4 * y - x) / 2, }
        : new List<int>();
