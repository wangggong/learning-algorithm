/*
 * @lc app=leetcode.cn id=design-underground-system lang=golang
 *
...
type UndergroundSystem struct {
	...
	CheckInInfo map[int]stationInfo
}
...
func Constructor() UndergroundSystem {
	return UndergroundSystem{
		...
		CheckInInfo: make(map[int]stationInfo),
	}
}

func (us *UndergroundSystem) CheckIn(id int, stationName string, t int) {
	// assert checkin once;
	us.CheckInInfo[id] = stationInfo{
		Station: stationName,
		Time:    t,
	}
}
...
func (us *UndergroundSystem) CheckOut(id int, stationName string, t int) {
	info := us.CheckInInfo[id]
	delete(us.CheckInInfo, id)
	k := info.Station + "-" + stationName
	us.Total[k] += t - info.Time
	us.Count[k]++
}

func (us *UndergroundSystem) GetAverageTime(startStation string, endStation string) float64 {
	k := startStation + "-" + endStation
	t, c := us.Total[k], us.Count[k]
