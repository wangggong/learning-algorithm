/*
 * @lc app=leetcode.cn id=rearrange-spaces-between-words lang=golang
 *
...
func reorderSpaces(text string) string {
	...
	for _, w := range words {
		if w != "" {
			realWords = append(realWords, w)
		}
	}
	spaces := strings.Count(text, " ")
	n := len(realWords)
	if n == 1 {
		return realWords[0] + strings.Repeat(" ", spaces)
	}
	ans := ""
	for i, w := range realWords {
		ans = ans + w
		if i+1 < n {
			ans = ans + strings.Repeat(" ", spaces/(n-1))
		}
	}
	return ans + strings.Repeat(" ", spaces%(n-1))
}
