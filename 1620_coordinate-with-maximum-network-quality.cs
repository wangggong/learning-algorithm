/*
 * @lc app=leetcode.cn id=coordinate-with-maximum-network-quality lang=csharp
 *
...
public class Solution
{
    ...
    public int[] BestCoordinate(int[][] towers, int radius)
    {
        ...
        for (int x = MinX; x <= MaxX; x++)
        {
            ...
            {
                int cur = GetQuality(towers, radius, x, y);
                if (cur > max)
                {
                    max = cur;
                    pos = new int[2] { x, y };
    ...
    private int GetQuality(int[][] towers, int r, int x, int y)
    {
        int ans = 0;
        foreach (var t in towers)
        {
            ...
            if (distSquare <= r * r)
            {
                ans += (int) Math.Floor((double) q / (1 + Math.Sqrt(distSquare)));
            }
        }
        return ans;
    }
