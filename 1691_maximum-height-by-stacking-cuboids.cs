/*
 * @lc app=leetcode.cn id=maximum-height-by-stacking-cuboids lang=csharp
 *
...
public class Solution
{
    ...
    {
        public int Compare(int[] x, int[] y) => x[2] != y[2]
            ? x[2].CompareTo(y[2])
            : (x[1] != y[1]
                    ? x[1].CompareTo(y[1])
                    : x[0].CompareTo(y[0]));
    ...
    public int MaxHeight(int[][] cuboids)
    {
        ...
        Array.Sort(cuboids, new Comparer());
        var n = cuboids.Count();
        var dp = new int[n];
        for (var i = 0; i < n; i++)
        {
        ...
        var ans = 0;
        foreach (var d in dp) { ans = Math.Max(ans, d); }
        return ans;
    }
}
