/*
 * @lc app=leetcode.cn id=queries-on-number-of-points-inside-a-circle lang=csharp
 *
...
{
    public int[] CountPoints(int[][] points, int[][] queries) => queries
        .Select(q => points
                .Where(p => (p[0] - q[0]) * (p[0] - q[0]) + (p[1] - q[1]) * (p[1] - q[1]) <= q[2] * q[2])
                .Count())
        .ToArray();
}
