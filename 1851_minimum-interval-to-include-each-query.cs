/*
 * @lc app=leetcode.cn id=minimum-interval-to-include-each-query lang=csharp
 *
...
public class Solution
{
    public int[] MinInterval(int[][] intervals, int[] queries)
    {
        ...
        var i = 0;
        foreach (var (q, j) in queries.Select((q, j) => (q, j))
            .OrderBy(x => x.q))
        {
            for (; i < n && intervals[i][0] <= q; i++)
            {
                Q.Enqueue(intervals[i], intervals[i][1] - intervals[i][0] + 1);
            }
            for (; Q.Count > 0; Q.Dequeue())
            {
                Q.TryPeek(out var v, out var _);
                if (v[1] >= q) { break; }
            }
            if (Q.Count > 0)
            {
