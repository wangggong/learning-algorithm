/*
 * @lc app=leetcode.cn id=eliminate-maximum-number-of-monsters lang=csharp
 *
...
{
    public int EliminateMaximum(int[] dist, int[] speed) => dist
        .Zip(speed, (d, s) => (d + s - 1) / s)
        .OrderBy(x => x)
        .ToList()
        .Select((l, i) => (l, i))
        .FirstOrDefault(x => x.l < x.i + 1, (l: 0, i: dist.Length))
        .i;
}
