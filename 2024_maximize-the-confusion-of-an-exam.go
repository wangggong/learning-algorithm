/*
 * @lc app=leetcode.cn id=maximize-the-confusion-of-an-exam lang=golang
 *
...
func maxConsecutiveAnswers(answerKey string, k int) int {
	getSize := func(p, q int) int { return q - p + 1 }
	AK := []byte(answerKey)
	n := len(AK)
	tCnt := 0
	ans := 0
	for p, q := 0, 0; q < n; q++ {
		...
		for ; p < n && tCnt > k && getSize(p, q)-tCnt > k; p++ {
			if AK[p] == 'T' {
				tCnt--
			}
		}
		if size := getSize(p, q); size > ans {
			ans = size
		}
	}
	return ans
