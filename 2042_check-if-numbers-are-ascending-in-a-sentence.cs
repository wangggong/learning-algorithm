/*
 * @lc app=leetcode.cn id=check-if-numbers-are-ascending-in-a-sentence lang=csharp
 *
...
{
    public bool AreNumbersAscending(string sentence)
    {
        var last = int.MinValue;
        foreach (var w in sentence.Split(" "))
        {
            if (int.TryParse(w, out var k))
            {
                if (last >= k)
                {
                    return false;
