/*
 * @lc app=leetcode.cn id=maximum-bags-with-full-capacity-of-rocks lang=golang
 *
 * NOTE (beewangruichao): I cannot use fid, thus I use slug instead.
 *
 * [2279] 装满石头的背包的最大数量
 *
 * https://leetcode-cn.com/problems/maximum-bags-with-full-capacity-of-rocks/description/
 *
 * algorithms
 * Medium (66.88%)
 * Total Accepted:    6.6K
...
func maximumBags(capacity []int, rocks []int, K int) int {
	...
	for i := 0; i < n; i++ {
		...
		notFull = append(notFull, capacity[i]-rocks[i])
	}
	sort.Ints(notFull)
	m := len(notFull)
	for i := 0; i < m && K >= notFull[i]; i++ {
		K -= notFull[i]
		ans++
	}
	return ans
