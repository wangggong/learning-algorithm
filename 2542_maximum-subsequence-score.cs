/*
 * @lc app=leetcode.cn id=maximum-subsequence-score lang=csharp
 *
...
public class Solution
{
    public long MaxScore(int[] nums1, int[] nums2, int k)
    {
        ...
        long cur = 0, ans = 0;
        foreach (var (i, v) in infos.OrderByDescending(x => x.Item2))
        {
            Q.Enqueue(nums1[i], nums1[i]);
            cur += (long)nums1[i];
            while (Q.Count > k)
            {
                cur -= (long)Q.Peek();
