/*
 * @lc app=leetcode.cn id=find-the-distinct-difference-array lang=csharp
 *
...
{
    public int[] DistinctDifferenceArray(int[] nums) => Enumerable
        .Range(0, nums.Length)
        .Select(i => nums[.. (i + 1)].ToHashSet().Count - nums[(i + 1) ..].ToHashSet().Count)
        .ToArray();
}
