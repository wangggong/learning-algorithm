/*
 * @lc app=leetcode.cn id=frequency-tracker lang=csharp
 *
...
public class FrequencyTracker
{
    ...
    private void ModifyCount(int n, int count)
    {
        Frequency[Counts[n]]--;
        Counts[n] = count;
        Frequency[Counts[n]]++;
    ...
    {
        Counts = new int[N + 1];
        Frequency = new int[N * 2 + 1];
    }

    public void Add(int n) => ModifyCount(n, Counts[n] + 1);

    public void DeleteOne(int n)
    {
        if (Counts[n] > 0)
        {
            ModifyCount(n, Counts[n] - 1);
        }
    }

    public bool HasFrequency(int frequency) => Frequency[frequency] > 0;
}
