/*
 * @lc app=leetcode.cn id=make-costs-of-paths-equal-in-a-binary-tree lang=csharp
 *
...
// 参考: https://leetcode.cn/problems/make-costs-of-paths-equal-in-a-binary-tree/solution/tan-xin-jian-ji-xie-fa-pythonjavacgo-by-5svh1/
//
// 树上贪心
public class Solution
{
    public int MinIncrements(int n, int[] costs)
    {
        ...
        for (var i = n >> 1; i > 0; i--)
        {
            ans += Math.Abs(costs[(i << 1) - 1] - costs[i << 1]);
            costs[i - 1] += Math.Max(costs[(i << 1) - 1], costs[i << 1]);
        }
        return ans;
    }
