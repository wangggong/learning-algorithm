/*
 * @lc app=leetcode.cn id=design-a-food-rating-system lang=golang
 *
...
// 参考: https://leetcode.cn/problems/design-a-food-rating-system/solution/ha-xi-biao-tao-ping-heng-shu-by-endlessc-hzct/
import (
	"github.com/emirpasic/gods/trees/redblacktree"
	"github.com/emirpasic/gods/utils"
)

type foodRatingInfo struct {
	food    string
	rating  int
	cuisine string
}
...
type FoodRatings struct {
	fMap map[string]foodRatingInfo
	cMap map[string]*redblacktree.Tree
}

func Constructor(foods []string, cuisines []string, ratings []int) FoodRatings {
	fr := FoodRatings{
		fMap: make(map[string]foodRatingInfo),
		cMap: make(map[string]*redblacktree.Tree),
	}
	n := len(foods)
	for i := 0; i < n; i++ {
		f, c, r := foods[i], cuisines[i], ratings[i]
		info := foodRatingInfo{f, -r, c}
		fr.fMap[f] = info
		if _, ok := fr.cMap[c]; !ok {
			fr.cMap[c] = redblacktree.NewWith(func(x, y interface{}) int {
				p, q := x.(foodRatingInfo), y.(foodRatingInfo)
				if p.rating != q.rating {
					return utils.IntComparator(p.rating, q.rating)
				}
				return utils.StringComparator(p.food, q.food)
			})
		}
		fr.cMap[c].Put(info, nil)
	}
	return fr
}
...
func (fr *FoodRatings) ChangeRating(food string, newRating int) {
	info := fr.fMap[food]
	newInfo := foodRatingInfo{food, -newRating, info.cuisine}
	fr.cMap[info.cuisine].Remove(info)
	fr.fMap[food] = newInfo
	fr.cMap[info.cuisine].Put(newInfo, nil)
	return
}

func (fr *FoodRatings) HighestRated(cuisine string) string {
	return fr.cMap[cuisine].Left().Key.(foodRatingInfo).food
}
