/*
 * @lc app=leetcode.cn id=minimum-amount-of-time-to-collect-garbage lang=golang
 *
 ...
 *
 */
func garbageCollection(arr []string, travel []int) (ans int) {
	n := len(arr)
	cnt := make([][3]int, n)
	last := []int{-1, -1, -1}
	for j, c := range []byte("GMP") {
		for i, a := range arr {
