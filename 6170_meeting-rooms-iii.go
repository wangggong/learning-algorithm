/*
 * @lc app=leetcode.cn id=meeting-rooms-iii lang=golang
 *
...
func (h Heap) Less(p, q int) bool {
	return h[p][1] < h[q][1] || h[p][1] == h[q][1] && h[p][0] < h[q][0]
}
...
func mostBooked(n int, M [][]int) (ans int) {
	sort.Slice(M, func(p, q int) bool { return M[p][0] < M[q][0] })
	cnt := make([]int, n)
	h := &Heap{}
	for i := 0; i < n; i++ {
		heap.Push(h, [2]int{i, 0})
	}
	for _, m := range M {
		var r [][2]int
		for h.Len() > 0 && (*h)[0][1] <= m[0] {
			r = append(r, heap.Pop(h).([2]int))
		}
		if len(r) == 0 {
			v := heap.Pop(h).([2]int)
			cnt[v[0]]++
			heap.Push(h, [2]int{v[0], m[1] - m[0] + v[1]})
			continue
		}
		sort.Slice(r, func(p, q int) bool { return r[p][0] < r[q][0] })
		cnt[r[0][0]]++
		r[0][1] = m[1]
		for _, rr := range r {
			heap.Push(h, rr)
