/*
 * @lc app=leetcode.cn id=total-cost-to-hire-k-workers lang=golang
 *
...
func (h Heap) Less(p, q int) bool {
	return h[p][0] < h[q][0] || h[p][0] == h[q][0] && h[p][1] < h[q][1]
}
...
func totalCost(costs []int, k int, candidates int) (ans int64) {
	...
	p, q := 0, n-1
	h := &Heap{}
	for i := 0; p <= q && i < candidates; i, p = i+1, p+1 {
		heap.Push(h, [2]int{costs[p], p})
	}
	for i := 0; q >= p && i < candidates; i, q = i+1, q-1 {
		heap.Push(h, [2]int{costs[q], q})
	}
	for i := 0; i < k; i++ {
		c := heap.Pop(h).([2]int)
		ans += int64(c[0])
