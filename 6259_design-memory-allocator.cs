/*
 * @lc app=leetcode.cn id=design-memory-allocator lang=csharp
 *
...
public class Allocator
{
    ...
    public int Allocate(int size, int mID)
    {
        ...
        for (; p < Cap; p = q)
        {
            for (; p < Cap && Cache[p] != 0; p++) { }
            for (q = p; q < Cap && q - p < size && Cache[q] == 0; q++) { }
            if (q - p == size) { break; }
        }
        if (q - p < size) { return -1; }
        for (var i = p; i < q; i++) { Cache[i] = mID; }
        return p;
    }
