/*
 * @lc app=leetcode.cn id=shortest-distance-to-target-string-in-a-circular-array lang=csharp
 *
...
public class Solution
{
    public int ClosetTarget(string[] words, string target, int startIndex)
        => words.ToList().IndexOf(target) < 0 ? -1 : Enumerable.Range(0, words.Length)
            .Where(i => words[i] == target)
            .Select(i => Math.Min(Math.Abs(i - startIndex), words.Length - Math.Abs(i - startIndex)))
            .Min();
}
