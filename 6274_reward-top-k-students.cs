/*
 * @lc app=leetcode.cn id=reward-top-k-students lang=csharp
 *
...
{
    public IList<int> TopStudents(string[] positiveFeedback, string[] negativeFeedback, string[] report, int[] studentId, int k)
    {
        var d = new Dictionary<string, int>();
        foreach (var w in positiveFeedback)
        {
            d[w] = 3;
        }
        foreach (var w in negativeFeedback)
        {
            d[w] = -1;
        }
        return Enumerable.Range(0, report.Length)
            .Select(i => (studentId[i], report[i]
                .Split(' ')
                .Select(s => d.ContainsKey(s) ? d[s] : 0)
                .Sum()))
            .OrderBy(x => -x.Item2)
            .ThenBy(x => x.Item1)
            .Take(k)
