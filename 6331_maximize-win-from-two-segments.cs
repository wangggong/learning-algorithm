/*
 * @lc app=leetcode.cn id=maximize-win-from-two-segments lang=csharp
 *
...
public class Solution
{
    public int MaximizeWin(int[] prizePositions, int k)
    {
        ...
        {
            for (; prizePositions[q] - prizePositions[p] > k; p++) { }
            var c = q - p + 1;
            ans = Math.Max(ans, c + prefix[p]);
            prefix[q + 1] = Math.Max(prefix[q], c);
        }
        return ans;
    }
}
