/*
 * @lc app=leetcode.cn id=design-graph-with-shortest-path-calculator lang=csharp
 *
...
public class Graph
{
    ...
    {
        N = n;
        G = Enumerable.Range(0, n).Select(_ => new int[n]).ToArray();
        Dist = Enumerable.Range(0, n).Select(_ => new int[n]).ToArray();
        for (var i = 0; i < N; i++)
        {
            Array.Fill(G[i], Maxn);
            G[i][i] = 0;
    ...
    public void AddEdge(int[] edge) => AddEdges(new int[][] { edge, });

    public int ShortestPath(int u, int v) => Dist[u][v] >= Maxn ? -1 : Dist[u][v];
}
