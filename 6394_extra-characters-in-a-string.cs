/*
 * @lc app=leetcode.cn id=extra-characters-in-a-string lang=csharp
 *
...
public class Solution
{
    public int MinExtraChar(string s, string[] dictionary)
    {
        ...
        int dfs(string s, int k)
        {
            ...
            memo[key] = dfs(s[1 ..], k + 1);
            foreach (var word in dictionary)
            {
                if (s.Length >= word.Length && s[.. word.Length] == word)
                {
                    memo[key] = Math.Min(memo[key], dfs(s[word.Length ..], k));
                }
            }
            return memo[key];
        }
