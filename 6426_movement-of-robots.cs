/*
 * @lc app=leetcode.cn id=movement-of-robots lang=csharp
 *
...
public class Solution
{
    ...
    {
        var ans = 0L;
        var (i, n) = (0,  nums.Length);
        foreach (var pos in nums.Zip(s, (n, s) => ((long)n + (long)d * (s == 'L' ? -1L : 1L))).OrderBy(x => x))
        {
            ans = (ans + (pos * (i - (n - 1 - i))) % Mod + Mod) % Mod;
            i++;
        }
        return (int)ans;
    }
