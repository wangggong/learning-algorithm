/*
 * @lc app=leetcode.cn id=lexicographically-smallest-palindrome lang=csharp
 *
...
public class Solution
{
    public string MakeSmallestPalindrome(string s)
    {
        ...
        for (var (i, j) = (0, chars.Length - 1); i < j; i++, j--)
        {
            if (chars[i] != chars[j])
            {
                var ch = chars[i] <= chars[j] ? chars[i] : chars[j];
                (chars[i], chars[j]) = (ch, ch);
            }
        }
