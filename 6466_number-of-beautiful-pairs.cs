/*
 * @lc app=leetcode.cn id=number-of-beautiful-pairs lang=csharp
 *
...
{
    private int Gcd(int x, int y) => y is 0
        ? x
        : Gcd(y, x % y);

    public int CountBeautifulPairs(int[] nums) => Enumerable
        .Range(0, nums.Length)
        .SelectMany(i => Enumerable
            .Range(i + 1, nums.Length - i - 1)
            .Where(j => Gcd(
                int.Parse(nums[i].ToString().Substring(0, 1)),
                nums[j] % 10) is 1))
        .Count();
}
