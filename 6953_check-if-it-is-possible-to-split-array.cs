/*
 * @lc app=leetcode.cn id=check-if-it-is-possible-to-split-array lang=csharp
 *
...
{
    public bool CanSplitArray(IList<int> nums, int m) => nums.Count() <= 2
        || Enumerable.Range(0, nums.Count() - 1)
            .Any(i => nums[i] + nums[i + 1] >= m);
}
